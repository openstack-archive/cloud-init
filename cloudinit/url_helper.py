...
class UrlResponse(object):
    ...
    def ok(self, redirects_ok=False):
        ...
        if redirects_ok:
            upper = _BAD_REQUEST
        if self.code >= OK and self.code < upper:
            return True
        else:
            return False
...
def read_url(url, data=None, timeout=None, retries=0,
    ...
    s = requests.Session()
    if retries:
        r = _Retry(total=max(int(retries), 0),
                   raise_on_redirect=not allow_redirects)
        s.mount(_get_base_url(url), adapters.HTTPAdapter(max_retries=r))
    try:
        with s:
            resp = s.request(**req_args)
            if check_status:
        ...
        LOG.debug("Read from %s (%s, %sb)", url, resp.status_code,
                  len(resp.content))
        return UrlResponse(resp)
