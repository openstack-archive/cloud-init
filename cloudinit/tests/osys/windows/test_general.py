...
class TestsWindowsGeneral(unittest.TestCase):
    ...
    def _test_check_os_version(self, ret_value, error_value=None):
        verset_return = 2
        windll_mock = self._windll_mock
        windll_mock.kernel32.VerSetConditionMask.return_value = (
            verset_return)
        windll_mock.kernel32.VerifyVersionInfoW.return_value = ret_value
        windll_mock.kernel32.GetLastError.return_value = error_value
        ...
        if error_value and error_value is not old_version:
            ...
            windll_mock.kernel32.GetLastError.assert_called_once_with()
            ...
            self.assertEqual(
                3, windll_mock.kernel32.VerSetConditionMask.call_count)

            mask = (self._kernel32.VER_MAJORVERSION |
                    self._kernel32.VER_MINORVERSION |
                    self._kernel32.VER_BUILDNUMBER)
            windll_mock.kernel32.VerifyVersionInfoW.assert_called_with(
