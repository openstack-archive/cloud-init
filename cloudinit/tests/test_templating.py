...
class TestTemplates(test.TestCase):
    ...
    def test_render_jinja(self):
        c = templater.render_string(self.j1tmpl, self.j1parms)
        self.assertEqual(self.j1exp, c)
    ...
    def test_jinja_without_jinja_raises_exception(self):
        blob = '\n'.join((
            ...
            ""))
        try:
            orig = templater.JINJA_AVAILABLE
            templater.JINJA_AVAILABLE = False
            self.assertRaises(
                ValueError, templater.render_string, blob, {'name': 'foo'})
        finally:
            templater.JINJA_AVAILABLE = orig

    def test_render_from_file(self):
        with test.TempDir() as td:
            fname = os.path.join(td, "myfile")
            with open(fname, "w") as fp:
                fp.write(self.j1tmpl)
            rendered = templater.render_from_file(fname, self.j1parms)

        self.assertEqual(rendered, self.j1exp)
    ...
    def test_render_to_file(self):
        with test.TempDir() as td:
            ...
            with open(src, "w") as fp:
                fp.write(self.j1tmpl)
            templater.render_to_file(src, target, self.j1parms)
